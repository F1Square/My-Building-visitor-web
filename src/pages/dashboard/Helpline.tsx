        ...
        <div className="space-y-3">
          {contacts.map(c => (
            <div key={c.id} className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 flex items-center gap-4">
              ...
              {canManage && (
                <button onClick={() => setDeleteId(c.id)} className="text-red-400 hover:text-red-600 p-2">
      ...
      <Dialog open={showForm} onOpenChange={setShowForm}>
        <DialogContent>
          ...
          <div className="space-y-3 mt-2">
            ...
            <Button className="w-full" disabled={submitting} onClick={handleAdd}>{submitting ? 'Adding...' : 'Add Contact'}</Button>
          </div>
        </DialogContent>
      </Dialog>

      <ConfirmDialog open={!!deleteId} onOpenChange={o => !o && setDeleteId(null)}
        title="Remove contact?" confirmLabel="Remove" onConfirm={handleDelete} />
    </div>
  );
}
